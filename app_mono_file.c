...
main(int argc, char *argv[])
{
	...
	//putting this as a local variable in load_bmp slowed
	//it down because it needed to be pushed onto the stack everytime
...
void load_bmp(char *file, char *skippableBuffer)
{
  ...
  //same, as exokernel, memory not being freed
  ...
  for(index = (b.height - 1) * b.width; index >= 0; index -= b.width)
    for( x = 0; x < b.width; x++)
      b.data[(ushort)index + x] =(uchar)read(fd, &c, sizeof(c));
